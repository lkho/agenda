...
module.exports = async function(jobName, definition) {
  ...
  if (s.topology.connections && s.topology.connections().length === 0 && !this._mongoUseUnifiedTopology) {
     ...
     */
    const JOB_RETURN_QUERY = {returnOriginal: true, sort: this._sort};
    ...
    if (result.value) {
      ...
      // if we acquired an expired lock, run the job now
      if (result.value.lockedAt != null) {
        result.value.nextRunAt = result.value.lastRunAt;
